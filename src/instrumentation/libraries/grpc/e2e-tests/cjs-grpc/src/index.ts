...
import { greeterClient, calculatorClient, userClient, grpcCallPromise, initializeClients } from './grpc/clients';
...
// Initialize gRPC clients AFTER instrumentation is set up
console.log('[Main] Initializing gRPC clients...');
initializeClients();

// Start gRPC server
console.log('[Main] Starting gRPC server...');
...
app.get('/greet/hello', async (req: Request, res: Response) => {
  try {
    ...
    const response: any = await grpcCallPromise(
      greeterClient,
      'SayHello',
      {
        name: 'World',
...
app.get('/test/sequential-calls', async (req: Request, res: Response) => {
  try {
    ...
    const greeting: any = await grpcCallPromise(
      greeterClient,
      'SayHello',
      { name: 'Sequential Test', greeting_type: 'formal' }
    );

    const calculation: any = await grpcCallPromise(
      calculatorClient,
      'Add',
      { num1: 5, num2: 10, operation: 'addition' }
    );

    const user: any = await grpcCallPromise(
      userClient,
      'GetUser',
      { id: 1 }
    );
