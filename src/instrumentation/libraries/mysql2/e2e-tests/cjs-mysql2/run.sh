...
PROJECT_NAME="mysql2-cjs-${APP_PORT}"

# Source common E2E helpers
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/../../../e2e-common/e2e-helpers.sh"

echo "Starting MySQL2 E2E test run on port ${APP_PORT}..."
...
echo "  - GET /test/knex-streaming"
docker compose -p $PROJECT_NAME exec -T app curl -s http://localhost:3000/test/knex-streaming > /dev/null
