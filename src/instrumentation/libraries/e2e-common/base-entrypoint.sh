...
set +e
TEST_OUTPUT=$(TUSK_ANALYTICS_DISABLED=1 TUSK_REQUIRE_INBOUND_REPLAY_SPAN=1 tusk drift run --print --output-format json --enable-service-logs 2>&1)
TUSK_EXIT=$?
set -e
...
ALL_PASSED=$(echo "$TEST_OUTPUT" | grep -c -E '"passed":[[:space:]]*true' || true)
