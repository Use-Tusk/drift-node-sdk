...
curl -sSf http://localhost:3000/test/basic-query > /dev/null
curl -sSf -X POST -H "Content-Type: application/json" -d '{"userId": 1}' http://localhost:3000/test/parameterized-query > /dev/null
curl -sSf http://localhost:3000/test/client-query > /dev/null
curl -sSf http://localhost:3000/test/client-connect > /dev/null
curl -sSf http://localhost:3000/test/client-close > /dev/null
curl -sSf http://localhost:3000/test/pool-query > /dev/null
curl -sSf -X POST -H "Content-Type: application/json" -d '{"userId": 2}' http://localhost:3000/test/pool-parameterized > /dev/null
curl -sSf http://localhost:3000/test/pool-connect > /dev/null
curl -sSf http://localhost:3000/test/pool-transaction > /dev/null
curl -sSf http://localhost:3000/test/query-rowmode-array > /dev/null
curl -sSf http://localhost:3000/test/multi-statement > /dev/null
sleep 3
kill $SERVER_PID 2>/dev/null || true
wait $SERVER_PID 2>/dev/null || true; sleep 2
