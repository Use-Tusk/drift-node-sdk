...
app.get("/types/bytes", async (req, res) => {
  try {
    ...
    const arrayBase64s = record.blobs.map((b: any) => Buffer.from(b).toString("base64"));
    res.json({
      isBuffer,
      length,
      base64,
...
app.get("/types/decimal-array", async (req, res) => {
  try {
    ...
    const allHaveToFixed = record.prices.every((p: any) => typeof p.toFixed === "function");
    const stringValues = record.prices.map((p: any) => p.toString());
...
// =============================================================================
// $queryRaw type deserialization tests — same types but via raw SQL.
// $queryRaw has no model name, so type reconstruction uses a different path
// (type map sniffed during recording) vs model-based operations above.
// =============================================================================

// Test $queryRaw BigInt deserialization
...
// Test $queryRaw Bytes deserialization
...
// Test $queryRaw DateTime deserialization
...
// Test $queryRaw Decimal deserialization
