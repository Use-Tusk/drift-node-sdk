...
export class MysqlInstrumentation extends TdInstrumentationBase {
  ...
  private _patchMysqlModule(mysqlModule: MysqlModuleExports): MysqlModuleExports {
    // NOTE: This method is disabled in favor of file-based patching (see init() method).
    // File-based patching ensures that Connection.prototype and Pool.prototype methods
    // are patched at the prototype level, which is necessary for transaction support.
    logger.debug(`[MysqlInstrumentation] Skipping module-level patching (using file-based patching instead)`);
    return mysqlModule;

    // Patch top-level functions
    if (mysqlModule.createConnection && typeof mysqlModule.createConnection === "function") {
      this._wrap(mysqlModule, "createConnection", this._patchCreateConnection());
      logger.debug(`[MysqlInstrumentation] Wrapped createConnection`);
    }

    if (mysqlModule.createPool && typeof mysqlModule.createPool === "function") {
      this._wrap(mysqlModule, "createPool", this._patchCreatePool());
      logger.debug(`[MysqlInstrumentation] Wrapped createPool`);
    }

    if (mysqlModule.createPoolCluster && typeof mysqlModule.createPoolCluster === "function") {
      this._wrap(mysqlModule, "createPoolCluster", this._patchCreatePoolCluster());
      logger.debug(`[MysqlInstrumentation] Wrapped createPoolCluster`);
    }

    this.markModuleAsPatched(mysqlModule);
    logger.debug(`[MysqlInstrumentation] MySQL module patching complete`);
  ...
  private _patchCreateConnection() {
    ...
    return (originalCreateConnection: Function) => {
      return function createConnection(this: any, connectionConfig: any) {
        const connection = originalCreateConnection.call(this, connectionConfig);

        // Wrap query method on the connection
        self._wrap(connection, "query", self._patchQueryMethod(connection, "connection"));

        // Wrap connect method to prevent TCP instrumentation alerts
        ...
        logger.debug(`[MysqlInstrumentation] Wrapped connection methods`);
        return connection;
      };
    };
  }
  ...
  private _patchCreatePool() {
    ...
    return (originalCreatePool: Function) => {
      return function createPool(this: any, poolConfig: any) {
        const pool = originalCreatePool.call(this, poolConfig);

        // Wrap query method on the pool
        self._wrap(pool, "query", self._patchQueryMethod(pool, "pool"));

        // Wrap getConnection to patch individual connections from the pool
        self._wrap(pool, "getConnection", self._patchGetConnection(pool));

        logger.debug(`[MysqlInstrumentation] Wrapped pool methods`);
        return pool;
      };
    };
  }
  ...
  private _patchCreatePoolCluster() {
    ...
    return (originalCreatePoolCluster: Function) => {
      return function createPoolCluster(this: any, poolClusterConfig: any) {
        const cluster = originalCreatePoolCluster.call(this, poolClusterConfig);

        // Wrap getConnection to patch connections from the cluster
        self._wrap(cluster, "getConnection", self._patchGetConnection(cluster));

        logger.debug(`[MysqlInstrumentation] Wrapped pool cluster methods`);
        return cluster;
      };
    };
  }

  private _patchGetConnection(poolOrCluster: any) {
    const self = this;
    return (originalGetConnection: Function) => {
      return function getConnection(this: any, callback?: Function) {
        const inputValue = { clientType: "pool" as const };
        ...
        if (self.mode === TuskDriftMode.REPLAY) {
          return handleReplayMode({
            ...
            replayModeHandler: () => {
              return SpanUtils.createAndExecuteSpan(
                self.mode,
                () => {
                  // Don't call originalGetConnection in replay mode - return mock directly
                  return self._handleReplayPoolGetConnection(null as any, callback);
                },
                {
                  name: `mysql.pool.getConnection`,
                  kind: SpanKind.CLIENT,
          ...
          // Disabled mode - just patch the connection methods without creating spans
          ...
          // Wrap the callback to patch the connection
          const wrappedCallback = function (this: any, err: any, connection: any) {
            if (connection && !err) {
              // Wrap query method on the pooled connection
              self._wrap(connection, "query", self._patchQueryMethod(connection, "poolConnection"));
              ...
              self._wrap(
                connection,
                "beginTransaction",
                self._patchBeginTransaction(connection),
              );
              self._wrap(connection, "commit", self._patchCommit(connection));
              self._wrap(connection, "rollback", self._patchRollback(connection));
            }
          ...
          return originalGetConnection.apply(this, [wrappedCallback]);
        }
      };
    };
  }

  private _patchQueryMethod(connection: any, clientType: "connection" | "pool" | "poolConnection") {
    const self = this;
    return (originalQuery: Function) => {
      return function query(this: any, ...args: any[]) {
        // Parse query arguments - MySQL supports multiple signatures:
