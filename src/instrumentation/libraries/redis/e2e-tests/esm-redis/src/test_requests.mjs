...
await makeRequest("POST", "/test/hset", {
  body: { key: "test:user:2", field: "name", value: "Jane Doe" },
});
...
await makeRequest("POST", "/test/set-with-expiry", {
  body: { key: "test:expiry", value: "expiry_value", seconds: 60 },
});
await makeRequest("POST", "/test/set-nx", { body: { key: "test:nx:unique", value: "nx_value" } });
