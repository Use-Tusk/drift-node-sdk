...
docker compose -p $PROJECT_NAME exec app curl -s http://localhost:3000/test-http-get > /dev/null

echo "  - POST /test-http-request"
docker compose -p $PROJECT_NAME exec app curl -s -X POST http://localhost:3000/test-http-request > /dev/null

echo "  - GET /test-https-get"
docker compose -p $PROJECT_NAME exec app curl -s http://localhost:3000/test-https-get > /dev/null

echo "  - GET /test-axios-get"
docker compose -p $PROJECT_NAME exec app curl -s http://localhost:3000/test-axios-get > /dev/null

echo "  - POST /test-axios-post"
docker compose -p $PROJECT_NAME exec app curl -s -X POST http://localhost:3000/test-axios-post > /dev/null
