...
# E2E Test Entrypoint for Firestore (CJS) Instrumentation
...
if [ -z "$FIREBASE_PROJECT_ID" ]; then
  log "ERROR: FIREBASE_PROJECT_ID environment variable is not set" "$RED"
  exit 1
fi

if [ -z "$FIREBASE_SERVICE_ACCOUNT" ]; then
  log "ERROR: FIREBASE_SERVICE_ACCOUNT environment variable is not set" "$RED"
  exit 1
fi
...
if [ -n "$BENCHMARKS" ]; then
  ...
  ENDPOINTS="/health /document/get /query/get"
